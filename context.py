...
class LoadingBar:
	...
	def __init__(self, bar=False, show_percent=True):
	...
	def print_bar(self):
		...
		while self.is_running:
			...
			for j in xrange(len(self.mapping)):
				sys.stdout.flush()
				progress = '({: >3}%) '.format(str(int(100*self.percentage_est)))
				string = "[" + "#"*self.n +self.mapping[j] + '.'*(self.N-1-self.n) + "] " + progress + t_remain
	...
	def est_remaining(self):
		...
		if self.percentage_est != 0:
			...
			min_terms = 10 # central limit theorem!
			if self.n > self.N / 2:
				min_terms = 1
