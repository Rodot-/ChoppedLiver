...
def tofits(outfilename, pixelarray, hdr=None, verbose=True, cols=None):
	"""
	...
	hdul.append(pyfits.PrimaryHDU(pixelarray.transpose(), hdr))

	if cols is not None: # must be a dict or iterator of 2-tuples

		iterator = cols.items() if type(cols) is dict else cols
		fits_cols = []
		
		for name, data in iterator:
			form='2048E'
			if data.dtype.name == "bool":
				data = np.cast["uint8"](data)
				form = '2048B'
			fits_cols.append(pyfits.Column(name=name, array=data.transpose(), format=form)) 

		fits_cols = pyfits.ColDefs(fits_cols)
		hdu = pyfits.BinTableHDU.from_columns(fits_cols)
